...
const fieldClass =
  "w-full rounded-lg bg-zinc-900 border border-amber-900/30 px-3 py-2 text-base text-zinc-100 placeholder-neutral-300 focus:outline-none focus:border-amber-700/60 transition-colors";
    ...
    <div>
      ...
      <div className="flex w-full items-center gap-2 rounded-lg border border-amber-900/30 bg-zinc-900 px-3 py-2">
        <input
          ...
          onBlur={handleBlur}
          className="flex-1 bg-transparent text-base text-zinc-100 placeholder-zinc-600 focus:outline-none"
          pattern="\d{4}-\d{2}-\d{2}"
        />
        <Popover>
          ...
          <PopoverContent className="w-auto p-0 bg-zinc-900 border-amber-900/40" align="start">
            <Calendar
              ...
              classNames={{
                ...
                dropdowns: "flex h-[--cell-size] w-full items-center justify-center gap-1.5 text-base font-medium",
                dropdown_root: "relative rounded-lg border border-amber-900/30 bg-zinc-900",
                caption_label: "flex items-center gap-1 px-2 py-1 text-base text-zinc-100 select-none",
  ...
  return (
    <div className="w-full flex flex-col gap-3">
      ...
      <form action={formAction} className="flex flex-col gap-3">
        {state?.error && (
          <p className="rounded-lg border border-red-800/60 bg-red-950/40 px-3 py-2 text-base text-red-400">
            {state.error}
          </p>
        )}
        ...
        <div className={sectionClass}>
          <div className="flex flex-col gap-2">
            <label className="text-base text-amber-600/90 tracking-widest uppercase hover:no-underline hover:text-amber-500/80 transition-colors [&>svg]:text-amber-700/60">
            ...
            <div className="flex gap-2 items-center">
              ...
              <button
                ...
                className="shrink-0 bg-amber-900/60 hover:bg-amber-800/70 disabled:opacity-50 text-amber-200 text-sm font-medium rounded-lg px-3 py-2 transition-colors"
              >
                    ...
                    {searchResults.map((m) => (
                      ...
                      >
                        {m.poster_path ? (
                          ...
                          <div className="w-8 h-12 bg-zinc-800 rounded flex items-center justify-center text-sm text-neutral-300 shrink-0">
                            No Img
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <p className="text-zinc-100 text-base font-medium truncate">{m.title}</p>
                          <p className="text-amber-600/90 text-sm">{m.release_date?.substring(0, 4)}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
        ...
        <Accordion type="single" collapsible value={accordionValue} onValueChange={setAccordionValue} className="bg-zinc-900/60 border border-amber-900/20 rounded-xl flex flex-col">
          <AccordionItem value="movie-details" className="border-none">
            <AccordionTrigger className="px-3 py-3 text-base text-amber-600/90 tracking-widest uppercase hover:no-underline hover:text-amber-500/80 transition-colors [&>svg]:text-amber-700/60">
              映画詳細（年代とジャンルを手動入力）
            </AccordionTrigger>
                ...
                <div className="flex w-full items-center gap-2 rounded-lg border border-amber-900/30 bg-zinc-900 px-3 py-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <button
                        type="button"
                        className="text-amber-600/90 hover:text-amber-500 transition-colors"
                        aria-label="年代一覧から選択"
                      >
                      ...
                      <div className="max-h-56 overflow-y-auto">
                        {yearOptions.map((y) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-1.5 text-base transition-colors ${(y === "不明" && year === "") || year === y
        ...
        <button
          type="submit"
          disabled={isPending}
          className="w-full py-3 rounded-xl bg-amber-800 hover:bg-amber-700 disabled:opacity-50 text-amber-100 font-semibold text-base tracking-wide transition-colors shadow-lg shadow-amber-950/30"
        >
